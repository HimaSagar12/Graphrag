...
def main():
    ...
    if uploaded_files:
        ...
        #                 user_msg=f"Analyze the following codebase and answer the query: '{query}' and the codebase is this {st.session_state.code_contents}"
        #             )
        ...
        # st.header("look for optimising opportunities")
        # query = "please look if you can find any code optimising opportunities, like parallel computing, reducing time complexity or space complexity"

        # if st.button("click to analise the code"):
        #     if query:
        #         client = HorizonLLMClient()
        #         response = "I couldn't understand your query. Try something like: 'functions in <file_name>', 'callers of <function_name>', 'details of <node_name>'."
        ...
        #         try:
        #             horizon_response = client.get_chat_response(
        #                 user_msg=f"task: '{query}' and the codebase is this {st.session_state.code_contents}"
        #             )
        #             response = horizon_response["model_answer"]
        #         except Exception as e:
        #             response = f"An error occurred while querying: {e}"
        ...
        #             )
        #             optimized_code = response["model_answer"]

        #             st.session_state.modified_files[file_name] = optimized_code

        #             st.subheader(f"Optimized Suggestions for {file_name}:")
